...
public class OutputView {
    ...
    private static final String GAME_RESULT_INFO_MESSAGE =  "최종 게임 결과";
    private static final String GAME_RESULT_MESSAGE_FORMAT = "게임 성공 여부: %s";
     ...
     */
    public void printResult(BridgeGame bridgeGame, Player player) {
        System.out.println(GAME_RESULT_INFO_MESSAGE);
        printMap(bridgeGame.getGameMap(), player);

        String statusMessage = bridgeGame.getStatus(player).toString();
        System.out.println(String.format(GAME_RESULT_MESSAGE_FORMAT, statusMessage));

        System.out.println(String.format(GAME_TRY_COUNT_MESSAGE_FORMAT, player.getTryCount()));
    }
}
